...
mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
{
    ...
    for (size_t i = 0; i < no_of_mempool_tx; ++i)
    {
        ...
        txs.push_back(mstch::map {
                ...
                {"rub_inputs"      , mempool_tx.xmr_inputs_str},
                {"rub_outputs"     , mempool_tx.xmr_outputs_str},
                ...
                bool tx_prove = false)
{
    ...
    {
        return string("Monero-Rub address not provided!");
    }
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            tx_context["outputs_rub_sum"] = xmreg::xmr_amount_to_str(outputs_xmr_sum);
            ...
            tx_context["inputs_rub_sum"] = xmreg::xmr_amount_to_str(inputs_xmr_sum);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    context.insert({"viewkey"        , REMOVE_HASH_BRAKETS(
            fmt::format("{:s}", prv_view_key))});
    context.insert({"has_total_rub"  , false});
    context.insert({"total_rub"      , string{}});
    context.insert({"key_imgs"       , mstch::array{}});
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    if (total_xmr > 0)
    {
        context["has_total_rub"] = true;
        context["total_rub"] = xmreg::xmr_amount_to_str(total_xmr);
    }
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    mstch::map context {
            {"rub_address"        , REMOVE_HASH_BRAKETS(address_str)},
            {"public_viewkey"     , REMOVE_HASH_BRAKETS(pub_viewkey_str)},
            {"public_spendkey"    , REMOVE_HASH_BRAKETS(pub_spendkey_str)},
            {"is_integrated_addr" , false},
            {"testnet"            , testnet},
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_rub_sum_not_zero");
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Monero-Rub address not provided";
        return j_response;
    }
...
get_tx_json(const transaction& tx, const tx_details& txd)
{
    ...
    json j_tx {
            ...
            {"mixin"       , txd.mixin_no},
            {"tx_size"     , txd.size},
            {"rub_outputs" , txd.xmr_outputs},
            {"rub_inputs"  , txd.xmr_inputs},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    context["inputs_rub_sum_not_zero"]  = (inputs_xmr_sum > 0);
    context["inputs_rub_sum"]           = xmreg::xmr_amount_to_str(inputs_xmr_sum);
    ...
    context["outputs_rub_sum"] = xmreg::xmr_amount_to_str(outputs_xmr_sum);
